...
def pension_precompute(par):
    ...
    for t in range(T):
        for ma in MA:
            for st in range(NST):
                for ra in RA:
                    pension_arr[t,ma,st,ra,:] = pension(t-par.ad_min,ma,st,ra,a,par)
...
def priv_pension(t,ma,st,a,par):
    ...
    priv[mask_a] = 0.0*a[mask_a]#frac*a[mask_a]
    return priv    
...
def labor_precompute(par):
    ...
    for t in range(Tr):
        for st in range(NST):
            ...
            # men
            pre_h = labor_pretax(t-ad_min,1,st,par)
            labor[t,1,st,:] = labor_posttax(t-ad_min,pre_h,spouse_inc,par,xi_men)
            
            # women
            pre_w = labor_pretax(t-ad_min,0,st,par)
            labor[t,0,st,:] = labor_posttax(t-ad_min,pre_w,spouse_inc,par,xi_women) 
                ...
                for st_h in range(NST):
                    for st_w in range(NST):
                        ...
                        labor_c[t,ad+ad_min,st_h,st_w,:] = (labor_posttax(t,pre_h,pre_w,par,xi_men_corr) + 
                                                            labor_posttax(t+ad,pre_w,pre_h,par,xi_women_corr))
...
def survival_precompute(par):
    ...
    for t in range(T):
        for ma in MA:
            survival_arr[t,ma] = survival(t-par.ad_min,ma,par)
